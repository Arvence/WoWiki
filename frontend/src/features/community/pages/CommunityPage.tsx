      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
        <section id="entries" className="scroll-mt-24" aria-label="Community entries">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3" aria-label="Community feed controls">
            <div className="flex items-center gap-1" role="group" aria-label="Community feed">{(['top', 'latest', 'event'] as const).map((item) => <button key={item} type="button" onClick={() => setFeed(item)} aria-pressed={feed === item} className={`rounded-md px-3 py-2 text-sm font-semibold capitalize ${feed === item ? 'bg-primary/15 text-primary' : 'text-muted hover:bg-background hover:text-text'}`}>{item}</button>)}</div>
            <div className="relative" role="group" aria-label="Listing type">
