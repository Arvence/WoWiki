      ...
      <main className="mx-auto w-full max-w-[100rem] px-3 py-5 sm:px-5 sm:py-7">
        <section className="overflow-hidden rounded-2xl border border-border/70 bg-surface/65 shadow-[0_16px_45px_rgba(0,0,0,0.2)]">
          ...
          <div className="flex min-h-14 flex-col gap-3 border-b border-border/60 bg-background/20 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            {activeCollection.searchable ? <label className="relative w-full sm:w-80"><span className="sr-only">Filter {activeCollection.title}</span><svg className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setQuery('') }} placeholder={`Filter ${active}...`} className="h-9 w-full rounded-lg border border-border bg-background/65 pl-9 pr-8 text-sm text-text outline-none placeholder:text-muted/60 focus:border-primary [&::-webkit-search-cancel-button]:hidden" />{query && <button type="button" onClick={() => setQuery('')} aria-label={`Clear ${activeCollection.title} filter`} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted hover:text-text focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">&times;</button>}</label> : <span className="text-xs text-muted">Browse-only reference</span>}
          </div>
