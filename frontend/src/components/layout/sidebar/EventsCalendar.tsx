...
export default function EventsCalendar(): JSX.Element {
  ...
  return (
    <section className="relative isolate overflow-hidden rounded-2xl bg-surface/75 shadow-[0_14px_40px_rgba(0,0,0,0.14)] md:grid md:grid-cols-[minmax(0,1fr)_17rem] md:items-stretch lg:block" aria-labelledby="classic-events-title">
      ...
      <div className="relative isolate overflow-hidden px-3 pb-3 pt-3 lg:px-4 lg:pb-4 lg:pt-4">
        <img
        ...
        <div className="relative z-10 flex items-start justify-between gap-3">
          <div>
            ...
            <h4 id="classic-events-title" className="mt-0.5 text-sm font-semibold text-text lg:mt-1 lg:text-base">WoW Classic Events</h4>
          </div>
        ...
        <div className="relative z-10 mt-3 flex items-center justify-between rounded-xl bg-background/35 p-1 shadow-inner shadow-black/5 lg:mt-4">
        ...
        <div className="relative z-10 mt-2 grid grid-cols-7 text-center lg:mt-3" aria-hidden="true">
          {weekDays.map((day) => <span key={day} className="py-1 text-[0.55rem] font-bold uppercase tracking-wider text-muted/75 lg:py-1.5 lg:text-[0.58rem]">{day}</span>)}
        </div>
          ...
          {calendarDays.map((date) => {
            ...
            return (
              <button
                ...
                className={`relative flex h-8 items-center justify-center rounded-lg text-[0.7rem] transition duration-200 lg:h-auto lg:aspect-square lg:rounded-full lg:text-xs ${selected ? 'scale-105 bg-primary font-bold text-background shadow-[0_5px_14px_rgba(212,160,74,0.28)]' : inMonth ? 'text-text hover:bg-background/65 hover:text-primary' : 'text-muted/30 hover:bg-background/35'} ${isToday && !selected ? 'font-bold text-primary ring-1 ring-inset ring-primary/60' : ''}`}
      ...
      <article className="relative mx-3 mb-3 rounded-2xl bg-background/40 p-3 shadow-inner shadow-black/5 md:mb-3 md:mt-3 lg:mx-3 lg:mb-3 lg:mt-0 lg:p-4" aria-live="polite">
        ...
        {selectedEvent ? <>
          ...
          <p className="mt-3 hidden text-xs leading-5 text-muted lg:block">{selectedEvent.description}</p>
          <div className="mt-3 hidden rounded-xl bg-surface/55 p-3 lg:block">
