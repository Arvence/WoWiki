      ...
      {news.map((article) => (
        <article key={article.id} className="overflow-hidden rounded-2xl border border-border bg-surface/80 shadow-lg">
          ...
          <div className="border-b border-border bg-background/50 p-4 sm:p-6">
            ...
            <div className="mt-4">
              <h2 className="text-2xl font-bold sm:text-3xl">
                <button type="button" className="text-left text-text transition hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                  {article.title}
                </button>
              </h2>
            </div>
          ...
          <div className="p-4 sm:p-6">
            ...
            <p className="mt-4 leading-relaxed text-muted">{article.content}</p>
          </div>
          ...
          <footer className="flex items-center border-t border-border/70 bg-background/30 px-4 py-3 sm:px-6">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-full text-muted transition hover:bg-surface-alt hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`View comments for ${article.title}`}>
                <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9.7 9.7 0 0 1-4.2-1L3 20l1.4-3.7A8.1 8.1 0 0 1 3 11.5a8.5 8.5 0 0 1 9-8.5 8.5 8.5 0 0 1 9 8.5Z" /><path d="M8 12h.01M12 12h.01M16 12h.01" strokeLinecap="round" strokeWidth="3" /></svg>
              </button>
            ...
            <button type="button" className="ml-auto inline-flex h-9 w-9 items-center justify-center rounded-full text-muted transition hover:bg-primary/10 hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Share ${article.title}`}>
