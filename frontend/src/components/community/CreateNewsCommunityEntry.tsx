  ...
  return (
    <>
      <div className="relative">
        <button type="button" onClick={() => setOpen(true)} className={compact ? 'inline-flex h-11 w-11 items-center justify-center rounded-full border border-primary-hover bg-primary text-background shadow-lg shadow-primary/20 transition-[background-color,box-shadow] duration-200 hover:bg-primary-hover hover:shadow-[0_0_0_3px_rgba(199,156,58,0.16),0_0_20px_rgba(199,156,58,0.38)] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background' : 'inline-flex items-center gap-2 rounded-full border border-primary-hover bg-primary px-4 py-2 text-sm font-bold text-background shadow-lg shadow-primary/20 transition-[background-color,box-shadow] duration-200 hover:bg-primary-hover hover:shadow-[0_0_0_3px_rgba(199,156,58,0.16),0_0_20px_rgba(199,156,58,0.38)] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary'} aria-label={`Create a community entry about ${newsTitle}`} title="Create community entry">
          ...
          {!compact && 'Create community entry'}
        </button>
      ...
      {open && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby={`community-entry-title-${newsId}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }}>
          <form onSubmit={(event) => void handleSubmit(event)} className="w-full max-w-lg rounded-2xl border border-border bg-surface p-5 shadow-2xl sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              ...
              <label className="grid gap-1.5 text-sm font-medium text-text">Your entry<textarea name="excerpt" required rows={4} placeholder="Share your thoughts..." className="resize-none rounded-lg border border-border bg-background px-3 py-2.5 text-text outline-none focus:border-primary" /></label>
            </div>
